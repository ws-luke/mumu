...
// 取得商品列表，並傳遞分頁和搜尋參數
const getProducts = async (page = 1, limit = 10, category = "", status = "" ) => {
  try {
...
const deleteProduct = async (id,title) => {
  try {
    ...
    if (result.isConfirmed) {
      ...
      await getProducts(pagination.value.currentPage, pagination.value.limit, selectedCategory.value, searchQuery.value);
    }
...
const endItem = computed(() =>
  Math.min(pagination.value.currentPage * pagination.value.limit, pagination.value.totalItems)
);

// 動態生成頁碼
const totalPages = computed(() => Array.from({ length: pagination.value.totalPages }, (_, i) => i + 1));
...
const changePage = async (page) => {
  ...
  if (page !== pagination.value.currentPage) {
    ...
    await getProducts(page, pagination.value.limit, selectedCategory.value, searchQuery.value);
  }
};
...
const fetchProducts = async () => {
  await getProducts(1, pagination.value.limit, selectedCategory.value, searchQuery.value);
};
...
<template>
  <div class="container-fluid">
            ...
            <table class="table table-borderless align-middle table-hover">
              ...
              <tbody>
                <tr v-for="product in productsData" :key="product.id">
                  ...
                  <td><span class="badge" :class="[ product.is_enabled ? 'bg-success' : 'bg-secondary']">{{ product.is_enabled ? '已上架' : '未上架' }}</span></td>
                  <td>{{ product.price }}</td>
    ...
    <div class="border-top d-md-flex justify-content-between align-items-center p-3">
      <div>顯示 {{ startItem }} - {{ endItem }} 共 {{ pagination.totalItems }} 筆</div>
      <nav>
        <ul class="pagination mb-0">
          ...
          <li v-for="page in totalPages" :key="page" class="page-item" :class="{ active: page === pagination.currentPage }">
